...
def start_rabbitmq_consumer():
    def callback(ch, method, properties, body):
        try:
            ...
            if data.get("status") == "DELIVERED" and order_id:
                order = find_order(order_id)
                if order and order.status in {"PAID", "DELIVERED"}:
                    order.status = "DELIVERED"
                    order.delivered_at = datetime.utcnow()
        except Exception as e:
            print(f"Failed to process delivery message: {e}")
    ...
    def consume():
        ...
        while retries > 0:
            try:
                ...
                print("Successfully connected to RabbitMQ. Waiting for messages...")
...
# Before replaying each interaction recorded by a consumer (Access Control),
# the Pact provider verifier POSTs the desired "state" to this endpoint so the
# in-memory order list can be seeded deterministically. This endpoint is never
# part of the production Ticketing API and is excluded from the OpenAPI docs.
# ---------------------------------------------------------------------------
from fastapi import Request as _Request


def _apply_ticketing_state(state: str | None) -> None:
    ORDERS.clear()
    if state and "paid order" in state.lower():
        ORDERS.append(
            Order(
                id="order-pact-001",
                customer_id="cust-pact",
                status="PAID",
                total=300.0,
                items=[
                    OrderItem(
...
if os.getenv("PACT_PROVIDER_STATES", "false").lower() == "true":

    @app.post("/_pact/provider_states", include_in_schema=False)
    async def setup_provider_state(request: _Request) -> dict:
        try:
